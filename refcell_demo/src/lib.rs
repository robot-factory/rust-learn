...
pub struct LimitTraker<'a,T:Messenger> {
    messenger: &'a T,
    value: usize,
    max: usize,
}

impl<'a,T>LimitTraker<'a, T> 
    where T: Messenger {
    pub fn new(messenger: &T, max: usize) -> LimitTraker<T> {
        LimitTraker {
            messenger,
            value: 0,
            max,
...
mod tests {
    ...
    fn it_sends_an_over_75_percent_warning_message() {
        ...
        let mut limit_tracker = LimitTraker::new(&mock_messenger,100);
